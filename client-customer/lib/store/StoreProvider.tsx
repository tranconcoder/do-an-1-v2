...
import { loginStart, loginSuccess, logout, User, Shop, LoginSuccessPayload } from './slices/userSlice'
    ...
    const initAuth = async () => {
      ...
      if (accessToken && currentRefreshToken) {
        ...
        try {
          ...
          const profileResponse = await axiosClient.get<ProfileApiResponse>('/user/profile', {
          ...
          dispatch(loginSuccess({ user: userData, shop: shopData, token: { accessToken: newAccessToken, refreshToken: newRefreshToken } }));
          console.log("StoreProvider: Profile fetch successful.");

        } catch (error: any) {
          console.warn("StoreProvider: Profile fetch failed with current token.", error.response?.data || error.message);
          if (error.response?.status === 401 && currentRefreshToken) {
            console.log("StoreProvider: Unauthorized. Attempting token refresh.");
            try {
              const refreshResponse = await axiosClient.post<RefreshTokenApiResponse>('/auth/refresh', { refreshToken: currentRefreshToken });
              ...
              console.log("StoreProvider: Token refresh successful. Re-fetching profile.");
              ...
              dispatch(loginSuccess({ user: refreshedUser, shop: refreshedShop, token: newTokens }));

            } catch (refreshError: any) {
              console.error("StoreProvider: Token refresh failed.", refreshError.response?.data || refreshError.message);
              dispatch(logout()); // This clears state and localStorage via the slice
            }
          } else {
            // Not a 401 or no refresh token, so just logout
            console.error("StoreProvider: Profile fetch failed (not 401 or no refresh token).");
            dispatch(logout());
          }
        }
      } else {
        console.log("StoreProvider: No tokens found in localStorage. Ensuring logged out state.");
        dispatch(logout()); 
      }
    };

    if (storeRef.current) { // Ensure store is initialized before running effect
        initAuth();
    }
