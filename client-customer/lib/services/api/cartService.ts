...
const cartService = {
  ...
  getCart: async (): Promise<CartItem[]> => {
    try {
      ...
      const flattenedItems: CartItem[] = response.data.metadata.flatMap(metadataItem =>
        metadataItem.cart_shop.products.map(product => ({
  ...
  removeItemFromCart: async (skuId: string): Promise<void> => {
    try {
      ...
      await apiClient.delete(`${CART_API_URL}/${skuId}`);
    } catch (error) {
