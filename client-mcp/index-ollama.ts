...
// Configuration
const OLLAMA_BASE_URL = process.env.OLLAMA_BASE_URL || "http://100.72.214.86:11434";
const MODEL_NAME = process.env.LLM_MODEL || "deepseek-r1:14b"; // Default Ollama model
...
Hãy hỏi tôi bất cứ điều gì về sản phẩm, dịch vụ, hoặc cách sử dụng website! 😊${contextInfo}`;
    ...
    async startChatLoop() {
        ...
        console.log("-".repeat(60));
        console.log(`🤖 Sử dụng model: ${MODEL_NAME} (qua Ollama Local)`);
        console.log(`🖥️ Ollama Server: ${OLLAMA_BASE_URL}`);
        ...
        // Note about local model capabilities
        console.log("🏠 Local Model: Tool calling hỗ trợ tùy thuộc vào model được sử dụng");
    ...
    showModelSuggestions() {
        console.log("\n🤖 GỢI Ý MODELS OLLAMA CHO ALICONCON MCP:");
        console.log("-".repeat(50));

        console.log("\n🏠 MODELS OLLAMA PHỔ BIẾN:");
        console.log("export LLM_MODEL='llama3.2:latest'        # Llama 3.2 (3B/1B)");
        console.log("export LLM_MODEL='llama3.1:8b'           # Llama 3.1 8B");
        console.log("export LLM_MODEL='llama3.1:70b'          # Llama 3.1 70B (cần RAM lớn)");
        console.log("export LLM_MODEL='qwen2.5:7b'            # Qwen 2.5 7B");
        console.log("export LLM_MODEL='qwen2.5:14b'           # Qwen 2.5 14B");

        console.log("\n🔧 MODELS HỖ TRỢ TOOL CALLING TỐT:");
        console.log("export LLM_MODEL='llama3.1:8b'           # Llama 3.1 có tool calling");
        console.log("export LLM_MODEL='qwen2.5:7b'            # Qwen 2.5 hỗ trợ function calling");
        console.log("export LLM_MODEL='mistral:7b'            # Mistral 7B");

        console.log("\n🚀 MODELS NHẸ CHO TESTING:");
        console.log("export LLM_MODEL='llama3.2:1b'           # Llama 3.2 1B (rất nhẹ)");
        console.log("export LLM_MODEL='phi3:mini'             # Phi-3 Mini");
        console.log("export LLM_MODEL='gemma2:2b'             # Gemma 2 2B");

        console.log("\n💡 Để thay đổi model:");
        console.log("1. Thoát chương trình (Ctrl+C)");
        console.log("2. Pull model: ollama pull <model_name>");
        console.log("3. Chạy lệnh export ở trên");
        console.log("4. Khởi động lại: bun run index-ollama.ts");

        console.log("\n🔗 Thay đổi Ollama server:");
        console.log("export OLLAMA_BASE_URL='http://your-server:11434'");
        console.log("-".repeat(50));
    }
...
async function main() {
    ...
    try {
        console.log(`🔍 Checking Ollama connection at ${OLLAMA_BASE_URL}...`);
        ...
        console.log(`✅ Connected to Ollama! Available models: ${models.models?.map((m: any) => m.name).join(', ') || 'None'}`);

        // Check if the specified model exists
        const modelExists = models.models?.some((m: any) => m.name === MODEL_NAME);
        if (!modelExists) {
            console.warn(`⚠️ Model '${MODEL_NAME}' not found. Available models: ${models.models?.map((m: any) => m.name).join(', ')}`);
            console.log("💡 To pull a model: ollama pull llama3.2");
        }
    } catch (error) {
        console.error("❌ Cannot connect to Ollama server!");
        console.log("💡 Hướng dẫn:");
        console.log("1. Đảm bảo Ollama đang chạy trên 100.127.25.47:11434");
        console.log("2. Kiểm tra kết nối mạng đến server");
        console.log("3. Thử: curl http://100.127.25.47:11434/api/tags");
        console.log(`4. Hoặc thay đổi OLLAMA_BASE_URL: export OLLAMA_BASE_URL=http://your-ollama-server:11434`);
        process.exit(1);
    }
